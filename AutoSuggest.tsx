...
const AutoSuggestComp: React.FC<Props> = ({
  ...
  const getSuggestions = (value: string, fetchedData: any[]) => {
    ...
    return inputLength === 0
      ...
      : fetchedData
      ? fetchedData.filter(
          (lang: { business_name: string }) =>
            lang.business_name.toLowerCase().slice(0, inputLength) ===
            inputValue
        )
      : []
  }
  ...
  const renderSuggestion = ({ business_name }: { business_name: string }) =>
    business_name
  ...
  const loadSuggestions = ({ value }: { value: string }) => {
    setLoading(true)
    axios({
      url: `unclaimed/?business_name__istartswith=${value}`,
      method: 'GET'
    })
      .then(({ data }) => {
        setLoading(false)
